...
def parse_make_model(input_string, output):
    ...
    for m in list(make_model):
        if re.search('([0-9][0-9][0-9][0-9])', m) is not None:
            output['year'] = m
            make_model.remove(m)
        elif re.search('year|type|sell|quick|sale|suv', m, re.IGNORECASE) is not None:
            make_model.remove(m)
...
class MySpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
             'https://avtobazar-ukraine.com.ua/en',
             ]
        for url in urls:
    ...
    def parse(self, response):
        ...
        product_links = response.xpath('//span[@class="art-PostHeader"]/a/@href').getall()
        yield from response.follow_all(product_links, self.detail)

        next_link = response.xpath('//li[@class="pager-next"]/a/@href').get()
    ...
    def detail(self, response):
        ...
        output['city'] = response.xpath('//div[@class="field field-type-content-taxonomy field-field-region"]/div/div/a/text()').get()
        output['ac_installed'] = 0
        output['engine_displacement_value'] = 0
        output['engine_displacement_unit'] = 0

        year = response.xpath('//div[@class="field field-type-number-integer field-field-year"]/div/div/text()').getall()[-1]
        if year is not None:
            output['year'] = int(re.sub("[^0-9]", "", year))
        mileage = response.xpath('//div[@class="field field-type-number-integer field-field-km"]/div/div/text()').getall()[-1]
        if mileage is not None:
            output['odometer_value'] = re.sub("[^0-9]", "", mileage)
            output['odometer_unit'] = 'km'

        make_model = ' '.join(response.xpath('//li[@class="lineage-0 odd first last"]/span/a/text()').getall())
        if make_model is not None:
            make_model = parse_make_model(make_model, output)


        cost = response.xpath('//div[@class="field field-type-number-integer field-field-price"]/div[@class="field-items"]/div/b/text()').getall()[-1]
        if cost is not None:
            cost = re.sub("[^0-9]", "", cost)
            output['price_wholesale'] = cost
            output['currency'] = 'USD'

        output['transmission'] = response.xpath('//div[@class="field field-type-text field-field-transmision"]/div/div/text()').getall()[-1].strip()


        picture_list = response.xpath('//span[@class="field-content"]/a/img/@src').getall()
        output['picture_list'] = ','.join(['https://avtobazar-ukraine.com.ua'+p for p in picture_list])
