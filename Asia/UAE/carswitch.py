...
class CarswitchcarsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            'https://carswitch.com/uae/used-cars/search',
        ]
        headers = {
    ...
    def parse(self, response, page):
        ...
        if '/uae/used-cars/' in page_link:
            page += 1
            page_link = f'https://carswitch.com/uae/used-cars/search?page={page}'
    ...
    def detail(self, response):
        ...
        if price_retail:
            output['price_retail'] = float(price_retail.replace('AED', '').replace(',', '').strip())
        output['currency'] = 'AED'
